...
public class MachineGun : WeaponPlayer
{
    ...
    public override void Shoot()
    {
        if (shootInput == 1f && Time.time > shotTimer)
        {
            ...
            {
                GameObject currentBullet = Instantiate(bullet, new Vector3(Random.Range(playerPos.x - spawnRange, playerPos.x + spawnRange), weaponPos.y + weaponLength, playerPos.z), Quaternion.Euler(0f, 0f, 90f));
            ...
            {
                GameObject currentBullet = Instantiate(bullet, new Vector3(weaponPos.x + weaponLength, Random.Range(weaponPos.y - spawnRange, weaponPos.y + spawnRange), playerPos.z), Quaternion.Euler(0f, playerRot, 0f));
